...
int auth_url_get_user_key(u08bits *usname, u08bits *realm, hmackey_t key)
{
	...
	if (ev) {
		TURN_LOG_FUNC(TURN_LOG_LEVEL_INFO, "Attempting authentication by performing a HTTP GET request to URL %s using CURL handle %p\n", this_url, curl);
	} else {
		TURN_LOG_FUNC(TURN_LOG_LEVEL_INFO, "HTTP GET %s\n", this_url);
	}
